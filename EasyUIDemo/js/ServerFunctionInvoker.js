...
function executeServerFunction(formId, className, methodName)
{
  ...
  if (AdminInst != null)
  {
	...
	//var connectionInfo = AdminInst.connectionInfo.toJSONString();  // original
    var connectionInfo = JSON.stringify(AdminInst.connectionInfo); // new
...
function updateResultFrame(className, methodName, resultObject)
{
  ...
  if (callCell != null && resultCell != null)
  {
    ...
    else
    {
      ...
      //var resultString = resultObject == null ? "[no result]" : resultObject.toJSONString(); // original
